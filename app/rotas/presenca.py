...
bp_presenca = Blueprint('presenca_api', __name__, url_prefix='/presenca')
presenca_schema = PresencaSchema(session=db.session)
presencas_schema = PresencaSchema(many=True, session=db.session)

@bp_presenca.route('/', methods=['GET'])
def listar_presencas():
    presencas = Presenca.query.all()
    return jsonify(presencas_schema.dump(presencas)), 200

@bp_presenca.route('/', methods=['POST'])
def criar_presenca():
    data = request.json
    try:
        nova_presenca = presenca_schema.load(data)
    except Exception as e:
        return jsonify({'error': str(e)}), 400
    db.session.add(nova_presenca)
    db.session.commit()
    return jsonify(presenca_schema.dump(nova_presenca)), 201
